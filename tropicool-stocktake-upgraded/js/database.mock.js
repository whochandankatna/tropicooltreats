/**
 * Mock data-access backend — in-memory, seeded from mock-data.js, nothing
 * here reaches a network or a real database. Every function is async and
 * returns plain objects/arrays, matching database.supabase.js's real
 * implementation function-for-function, so database.js can pick either one
 * at module load time based on whether SUPABASE_URL is set (config.js)
 * without any calling code in home.js/stocktake.js/etc. knowing which
 * backend is active.
 */
...
export async function postAnnouncement({ storeId, message, staffId, staffName }) {
  const a = { id: uid('ann'), storeId: storeId ?? null, message, staffId, staffName: staffName || null, createdAt: new Date().toISOString() };
